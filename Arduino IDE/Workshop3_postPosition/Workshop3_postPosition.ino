...
//--- Credentials for WiFi - Replace the variables with your SSID/Password combination
  ...
  const char* password = "sandwichmaker";
...
const char* mqttUser = "jokerofun";
const char* mqttPassword = "aio_AVYz02SF3d0kzYAmFkn1U2x05CDI";
...
void setup(){
  ...
  // connect wifi
    ...
    while (!client.connected()){
      ...
      if(client.connect("jokerofun", mqttUser,mqttPassword)){
        Serial.println("connected");
        client.subscribe("jokerofun/feeds/alert");
      } else {
    ...
    client.publish("jokerofun/feeds/alert", "{\"value\": \"Simeon\", \"lat\": \"57\", \"lon\": \"10\"}");
}
  ...
  Sensor = digitalRead(PIR);
    if (Sensor == HIGH) {          
      if (State == LOW) {
        ...
        String MQTTData = "{\"name\":\"Simeon\",\"type\":\"motion\",\"value\":\"trigged\",\"lat\":\"" + String(gps.location.lat()) + "\",\"lon\":\"" + String(gps.location.lng()) + "\"}";
        MQTTData.toCharArray(message_buffer, MQTTData.length()+1);
        client.publish("jokerofun/feeds/alert",message_buffer);
      ...
      if (State == HIGH) {
        ...
        String MQTTData = "{\"name\":\"Simeon\",\"type\":\"motion\",\"value\":\"cleared\",\"lat\":\"" + String(gps.location.lat()) + "\",\"lon\":\"" + String(gps.location.lng()) + "\"}";
        MQTTData.toCharArray(message_buffer, MQTTData.length()+1);
        client.publish("jokerofun/feeds/alert",message_buffer);
